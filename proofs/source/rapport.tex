...
\[
    \mathbb P(d_i \in [d_i^l - \gamma/2, d_i^l + \gamma/2]) \geq 1 - \exp\left( -\dfrac{1}{6}n\gamma^2 \right) - \exp\left( -\dfrac{1}{12}n\gamma^2 \right)\\
\]
...
Avec 
\[
    \alpha = \dfrac{8\left( \log k + \log(2/\beta) \right)}{\varepsilon}  \ \wedge \ \mu = 2\exp\left( - \dfrac{n\gamma}{4}\left(1 - \dfrac{2\alpha}{n\gamma}\right)^2\right) \ \wedge \ \eta  = \exp\left( -\dfrac{1}{6}n\gamma^2 \right) + \exp\left( -\dfrac{1}{12}n\gamma^2 \right)
\]
...
On pose alors \(A = \) ``il y a au moins \(in/10\) valeurs plus petites que \(d^l_i - \gamma/2\)'' et \(B = \) ``il y a au moins \((10-i)n/10\) valeurs plus grandes que \(d^l_i - \gamma/2\)''. Pour tout \(j \in \inte 0 {n-1}\) on pose \(A_j = \un_{x_j \leq d^l_i + \gamma/2}\), \(B_j = \un_{x_j \geq d^l_i - \gamma/2}\), \(A_s = \sum_{j = 0}^{n-1} A_j\) et \(B_s = \sum_{j = 0}^{n-1}\). On a alors, \(A = \{A_s \geq in/10\}\) et \(B = \{B_s \geq  n - in/10\}\). Une application d'une borne de {\sc Chernoff} assure alors que 
\begin{align*}
    \mathbb P(A) & = \mathbb P (A_s \geq in/10)\\
    & = \mathbb P \left(A_s \geq \dfrac{n}{\sqrt{2\pi}}\int_{-\infty}^{d^l_i + \gamma/2}   \exp\left( \dfrac{-t^2}{2} \right)\dt t  \left( 1 + \dfrac{i\sqrt{2\pi}}{10\int_{-\infty}^{d^l_i + \gamma/2}   \exp\left( {-t^2}/{2} \right)\dt t}  - 1\right)\right)
\end{align*}
